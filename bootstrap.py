...
firsttgt="all : version_info download chown_dir mk_env mk_tools mk_virt_file mk_chroot \
...
version_info: .FORCE
	@echo -e $(BOLD)$(BLUE) BUILDING LFS ''' + lfs.version.replace("\n","") + '''$(BOLD)

download:
	@$(call echo_message, Building )
	@$(call echo_summary, Downloading Sources Packs...please Be Patient and Look Into $(LOGDIR) for logs.)
...
chown_dir: FORCE
	$(call echo_message, Building)
	@$(call echo_summary, Change Ownership of LFS folders back to lfs user...if lfs user exisits.)
...
mk_env : download
	@$(call echo_message, Building)
	@$(call echo_summary, Building Chapter 4.)
	@make ''' + lfs.chapters[4].name + '''
	@touch $@

mk_tools: mk_env
	@$(call echo_message, Building)
	@$(call echo_summary, Building Chapter 5.)
...
mk_chroot : mk_virt_file
	@$(call echo_message, Building)
	@$(call echo_summary, Building Chapter 6.)
	@$(CHROOT1) "cd $(MAKEDIR) && make ''' + lfs.chapters[6].name + '''"
	@touch $@

mk_config : mk_chroot
	@$(call echo_message, Building)
	@$(call echo_summary, Building Chapter 7.)
	@$(CHROOT2) "cd $(MAKEDIR) && make ''' + lfs.chapters[7].name + '''"
	@touch $@

mk_boot : mk_config 
	@$(call echo_message, Building)
	@$(call echo_summary, Building Chapter 8.)
	@$(CHROOT2) "cd $(MAKEDIR) && make ''' + lfs.chapters[8].name + '''"
	@touch $@

mk_end : mk_boot
	@$(call echo_message, Building)
	@$(call echo_summary, Building Chapter 9.)
	@$(CHROOT2) "cd $(MAKEDIR) && make ''' + lfs.chapters[9].name + '''"
	@touch $@

mk_extra : mk_end 
	@$(call echo_message, Building)
	@$(call echo_summary, Building Chapter 10.)
...
mk_blfs : mk_end
	@$(call echo_message, Building)
	@$(call echo_summary, Generating Necessary Files for building BLFS.)
...
.PHONY: clean FORCE .FORCE umount_all chown_dir version_info
