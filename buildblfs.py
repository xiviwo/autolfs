...
globalreplace = [
		...
		('&amp;',						'&'),
		('<PREFIX>',						'/opt'),
		("useradd -m <newuser>",				'useradd -m mao'),
		("<username>",						'mao'),
		("<password>",						'ping'),
			...
			for child in child_iter:
				if  hasattr(child,"name"):
					...
					if child.has_attr("title"):
							...
							pkgname = funcstrip.sub("-",child['title']).lower()
							
							shortname = parsename(pkgname)
							version = parseversion(child['title'].lower())	
		...
		if type(cmd) is not str:
			...
			if cmd.parent.findPreviousSibling():
				...
				if containsAny(prev, ['doxygen','Doxygen','texlive','TeX Live','Sphinx']) and not containsAny(prev,['Install Doxygen']):
					...
					lineadd(cmdstr,cmd.text.encode('utf-8'),shortname + "-" + version) # cmd.text.encode('utf-8').strip() + "\n" #
		else:
			cmdstr.append(cmd)
	if packname:	
	...
	for title in titles:
		...
		while title != None:
			...
			if title != None :#and #func == "Crypt_SSLeay_0_64" :
				#pid+=int(pid) + 1
				perldepend(title,"",pid,cmds,pkglist)
...
def perldepend(div,func,pid,cmds,pkglist):
			 ...
			 if ul.string == None:
				...
				for li in ul.contents:  # Iterate over div of class itemizedlist
					if li.string == None and li.div:
						...
						perldepend(li.div,li.div.findPreviousSibling().a,pid,cmds,pkglist)
...
def genscript(link,pid,cmds):
	...
	if link.has_attr("href") and not containsAny(pkgname, ['conventions-used-in-this-book','notes-on-building-software','locale-related-issues']) :
	...
	elif pid  == '1324' or '2421': #for python modules/ Xorg drivers
		soupsub = link.parent
...
def parsemultisect(link,pid,cmds):
...
def parseindex():
	...
	for title in titles:
		...
		for ul in uls:
			...
			pcounter = 0
			for link in links:
				packid = str(ccounter) + str(pcounter).zfill(2)

				if link.string in "Perl Modules":
					for pkg in parseperl(packid,cmds):
						subchapter += pkg + " "
				if containsAny(link.string,["Python Modules","Xorg Drivers"]):
					subchapter  += parsemultisect(link,packid,cmds)
				else:
					subchapter  += genscript(link,packid,cmds) + " "
				pcounter +=1 # loop for packages
			subchapter += "\n"
