...
Requires:  libffi
Requires:  python
Source0:    http://llvm.org/releases/3.3/llvm-3.3.src.tar.gz
Source1:    http://llvm.org/releases/3.3/cfe-3.3.src.tar.gz
Source2:    http://llvm.org/releases/3.3/compiler-rt-3.3.src.tar.gz
...
%build
cd %{srcdir}
tar -xf  %_sourcedir/cfe-3.3.src.tar.gz -C tools 
tar -xf  %_sourcedir/compiler-rt-3.3.src.tar.gz -C projects 
mv tools/cfe-3.3.src tools/clang 
mv projects/compiler-rt-3.3.src projects/compiler-rt
sed -e 's:\$(PROJ_prefix)/docs/llvm:$(PROJ_prefix)/share/doc/llvm-3.3:' -i Makefile.config.in 
...
for file in ${RPM_BUILD_ROOT}/usr/lib/lib{clang,LLVM,LTO,profile_rt}*.a
