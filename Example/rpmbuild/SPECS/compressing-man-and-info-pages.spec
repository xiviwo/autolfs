...
Summary:    compressing-man-and-info-pages
Name:       compressing-man-and-info-pages
Version:    1.0
Release:    %{?dist}7.4
License:    GPLv3+
...
URL:        http://www.linuxfromscratch.org/blfs/view/stable/postlfs/compressdoc.html
%description
...
function help ()
{
  ...
  ( echo "Usage: $MY_NAME <comp_method> [options] [dirs]" && cat << EOT
...
if [ -z "$MAN_DIR" ]; then
  MAN_DIR=`manpath -q -C "$MAN_CONF"/man_db.conf | sed 's/:/\\n/g' | while read foo; do dirname "$foo"/.; done | sort -u | while read bar; do echo -n "$bar "; done`
fi
...
for DIR in $MAN_DIR; do
  ...
  for FILE in *; do
    ...
    if [ -d "$FILE" ]; then
      ...
      (cd "$MEM_DIR" && eval "$0" ${COMP_METHOD} ${COMP_LVL} ${LN_OPT} ${VERBOSE_OPT} ${FORCE_OPT} "${DIR}/${FILE}") || exit $?
      ...
      if [ -h "$FILE" ]; then
        ...
        if [ "$LN_OPT" = "-H" ]; then
          ...
          rm -f "$FILE" && ln -svf "${LINK}$COMP_SUF" "${FILE}$COMP_SUF"
        fi
        ...
        if [ -n "$HLINKS" ]; then
          for i in $HLINKS; do
            ...
            if [ "$LN_OPT" = "-S" ]; then
              ...
              ln -svf "${FILE}$COMP_SUF" "${NEWFILE}$COMP_SUF"
            else
