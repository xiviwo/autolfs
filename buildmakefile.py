...
def parseindex(index,file):
	...
	for title in titles:
		...
		chapters = "chapter" + str(counter) + ".mk"
		chaptermkfile = CWD+"/" + chapters
		file = open(chaptermkfile,'wb')
		...
		for ul in uls:
			...
			for link in links:	
				func =  funcstrip.sub("_",link.string)
				print "Parseing: ",func
				rules=""
				...
				if link.has_key('href'):
					...
					if "Perl_Modules" in func:
						rules += parseperl(sublink)
					else:
					#elif "D_Bus" in func:	
						rules += parsepage(sublink,func,str(counter))
				if rules:
					target += func + " "
					file.write(rules)
